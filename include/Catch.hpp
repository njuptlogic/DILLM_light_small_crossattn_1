...
namespace Catch {
    ...
    FatalConditionHandler::FatalConditionHandler() {
        ...
        sigStack.ss_sp = altStackMem.data();  // 使用 data() 获取指针
    ...
    std::vector<char> FatalConditionHandler::altStackMem(SIGSTKSZ);
    // 在使用之前声明并初始化 stackSize
    //size_t stackSize = SIGSTKSZ;  // 或者从 sysconf 获取栈大小

    // 然后使用 stackSize 来分配内存
    //char* altStackMem = new char[stackSize];
