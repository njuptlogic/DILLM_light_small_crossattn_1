...
class Param:
    def __init__(self):
        ...
        self.parser.add_argument('--seed', type=int, default=None, help='Random seed for reproducibility (default: None = non-deterministic)')
        ...
        self.parser.add_argument('--criticLr', dest='critic_lr', type=float, default=None,
                            help="Critic learning rate (default: same as --lr)")
        self.parser.add_argument('--lrMin', dest='lr_min', type=float, default=1e-5,
                            help="Minimum learning rate for cosine annealing scheduler")
        self.parser.add_argument('--cosineAnnealing', dest='cosine_annealing', action='store_const',
                            default=False, const=True, help="Enable cosine annealing LR scheduler")
        ...
        self.parser.add_argument("--entropyCoef", dest="entropy_coef", default=0.01, type=float,
                            help='Entropy regularization coefficient for policy gradient')
        ...
        self.parser.add_argument("--fusionProj", dest="fusion_proj", action="store_const",
                            default=False, const=True,
                            help="Enable gate-based FusionProjection in decoder (default: off, use original features)")
        self.parser.add_argument("--crossAttn", dest="cross_attn", action="store_const",
                            default=False, const=True,
                            help="Enable language-conditioned visual attention in decoder")

        # Resume best_val baselines (for continuing training across runs)
        self.parser.add_argument("--bestValSeen", dest="best_val_seen", type=float, default=None,
                            help="Override initial best val_seen SR (e.g. 0.547)")
        self.parser.add_argument("--bestValUnseen", dest="best_val_unseen", type=float, default=None,
                            help="Override initial best val_unseen SR (e.g. 0.490)")
